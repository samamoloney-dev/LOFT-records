...
router.get('/', async (req, res) => {
  ...
  const members = await Promise.all(rows.map(rowToCamel).map(withCurrency));
  res.json(members);
});
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      ...
      [d.firstName, d.lastName, d.type, d.role, d.fleet, d.type === 'PILOT' ? (d.lineCheckAnchorDate || null) : null],
    );
    const member = rowToCamel(rows[0]);
...
router.patch('/:id', async (req, res) => {
  ...
  res.json(await withCurrency(rowToCamel(rows[0])));
});
